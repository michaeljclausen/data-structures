describe('tree', function() {
  ...
  it('should add child to parent of same value', function() {
    tree = Tree(5);
    tree.addChild(5);
    expect(tree.children[0].value).to.equal(5);
    expect(tree.value).to.equal(5);
  });
  
  it('should add two children with the same value', function() {
    tree.addChild(5);
    tree.addChild(5);
    expect(tree.children[0].value).to.equal(5);
  ...
  it('should traverse every node and call a callback on that node', function() {
    let count = 0;
    cb = function(){
      count++;
    }
